...
clear *B        % *P if keeping PFC, *B if keeping BLA cells

a_nb_neu=who ('SPK*');      % get total nb of neurons

%%
LFP_name=input('Channel_1 (use quotes): ');
freqband=[2 5];
spike_prefix='SPK';
preflength=length(spike_prefix);
lfp_prefix='FP';
%freqband=[1 3;2 5;6 12;15 35;55 65;70 90];
...
for n=1:length(worksp)
    ...
    if length(nme)>=5
        ...
        if strcmp(sprintf(spike_prefix),sprintf('%s',nme(1,1:preflength)))==1;
            j=j+1;
            figure;
...
%                     pphi(i,j)=phi;
%                     axis tight
%                     eval(['pha' num2str(worksp(n,1).name(1,preflength+1:end)) '=[pval,phi(1,1)];']);
%                     phist(i,j,:)=h;
...
%                                 eval(['neu' num2str(worksp(n,1).name(1,preflength+1:end)) '=neu;']);
%                                 eval(['phi' num2str(worksp(n,1).name(1,preflength+1:end)) '{' num2str(i) '}=phaseneu;']);
%                                 
%                 end
                if length(freqband)==2
                    bar([-180:360/((ndeg-1)):570],cat(2,hist(phaseneu,ndeg),hist(phaseneu,ndeg))); axis tight; hold on
                    ...
                    eval(['phi' num2str(worksp(n,1).name(1,preflength+1:end)) '{' num2str(i) '}=phaseneu;']);
                    %eval(['phi' num2str(worksp(n,1).name(1,preflength+1:end)) '{' 1 '}=[phaseneu];']);
                    eval(['neu' num2str(worksp(n,1).name(1,preflength+1:end)) '=neu;']);
...
fLFP2=fLFP((1:length(fLFP)/2),:);

close all
clearvars -except ppval pphi pmrl pkappa pZ phistnorm phist

% save(path, phistnorm ppval pphi pmrl pZ pkappa); 
% save(path,ppval)